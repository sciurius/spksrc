...
NEXTCLOUD="${INSTALL_DIR}/bin/nextcloud.sh"
PID_FILE="${INSTALL_DIR}/var/nextcloud.pid"


start_daemon ()
{
    start-stop-daemon -S -q -m -b -N 10 -x ${NEXTCLOUD} -c ${USER} -u ${USER} -p ${PID_FILE} > /dev/null
}
