...
preinst ()
{
    if [ "${SYNOPKG_PKG_STATUS}" == "INSTALL" ]; then
        ...
        # Check directory
        if [ ! -d ${wizard_nextcloud_datadirectory:=/volume1/nextcloud} ]; then
...
postinst ()
{
    ...
    if [ "${SYNOPKG_PKG_STATUS}" == "INSTALL" ]; then
        ${MYSQL} -u root -p"${wizard_mysql_password_root}" -e "CREATE DATABASE ${MYSQL_DATABASE}; GRANT ALL PRIVILEGES ON ${MYSQL_DATABASE}.* TO '${MYSQL_USER}'@'localhost' IDENTIFIED BY '${wizard_mysql_password_nextcloud:=nextcloud}';"
        sed -i -e "s/@admin_username@/${wizard_nextcloud_admin_username:=admin}/g" \
               -e "s/@admin_password@/${wizard_nextcloud_admin_password:=admin}/g" \
               -e "s/@db_password@/${wizard_mysql_password_nextcloud:=nextcloud}/g" \
               -e "s#@directory@#${wizard_nextcloud_datadirectory:=/volume1/nextcloud}#g" \
               ${WEB_DIR}/${PACKAGE}/config/autoconfig.php
        chown -R ${USER} ${wizard_nextcloud_datadirectory}
    fi
